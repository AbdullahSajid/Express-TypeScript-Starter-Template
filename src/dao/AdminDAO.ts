...
class AdminDAO {
  ...
  async findOneByFilter(filter: any, attributes?: any, throwErrorIfNotExists = false): Promise<IAdmin | null> {
    const admin = await Admin.findOne(filter).select(attributes).lean()
    if (!admin && throwErrorIfNotExists) throw new ErrorHandler(404, "Admin does not exist")
    return admin
  }
  ...
  async updateById(id: string, data: any): Promise<void> {
    ...
    if (!res.matchedCount) throw new ErrorHandler(404, "Admin does not exist")
    if (!res.acknowledged) throw new ErrorHandler(500, "An error occured during updation")
  }
  ...
  async deleteById(id: string): Promise<void> {
    ...
    if (!res.acknowledged) throw new ErrorHandler(500, "An error occured during deletion")
  }
